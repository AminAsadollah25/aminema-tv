...
قبل از هر کاری این سه را کامل بخوان — قوانین، تصمیم‌های گرفته‌شده و تاریخچه
باگ‌های واقعی آن‌جاست و بدون آن‌ها دوباره‌کاری می‌کنی:

1. `AGENTS.md` — قرارداد دائمی حفظ پروژه و روش امن تغییر
...
6. **دو تصویر متفاوت برای هر عنوان:** `thumbnail` عمودی (~۰.۸:۱) برای کارت و
   foreground Hero، `cover` عریض (~۱.۸:۱) فقط برای backdrop/بنر. جای هم
   استفاده نمی‌شوند؛ Stretch کردن cover داخل قاب پوستر ممنوع است.
7. **Metadata Provider-first است:** cache و Provider با Merge غیرمخرب؛
   Wikipedia/Wikidata فقط فیلد خالی را با تطبیق معتبر تکمیل می‌کند. IMDb
   Scrape، داده ساختگی و ارسال Cookie/Token/media URL ممنوع است.
