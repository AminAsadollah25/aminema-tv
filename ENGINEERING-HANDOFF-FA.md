...
| **0.11.0** | فیکس race شرطی Direct Play در SPA، سقف Sync از ۱۲ به ۳۰، `quickLinks[]` برای یوتیوب فارسی. **پخش زنده در 0.11.0 اشتباه بود** (فقط لینک به صفحهٔ خودِ سایت) — کاربر بعداً طرح درست را خواست: ردیف بومی + فول‌اسکرین با یک کلیک؛ در ROADMAP.md بخش «پخش زنده» بازطراحی ثبت شده، هنوز پیاده نشده |
